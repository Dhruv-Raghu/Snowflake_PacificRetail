-- Create the JSON file format since product data is stored as a json file
CREATE OR REPLACE FILE FORMAT json_file_format
    TYPE = JSON
    STRIP_OUTER_ARRAY = TRUE
    IGNORE_UTF8_ERRORS = TRUE;

-- -- test the json file format to see if it is working
-- SELECT $1
-- FROM @adls_stage/Product/(FILE_FORMAT => json_file_format)
-- LIMIT 10;
...
-- Create task to schedule the automatic data transfer from azzure to table
